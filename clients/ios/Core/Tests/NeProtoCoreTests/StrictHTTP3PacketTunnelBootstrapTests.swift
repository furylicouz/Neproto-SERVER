...
@Suite("Strict HTTP/3 Packet Tunnel bootstrap")
struct StrictHTTP3PacketTunnelBootstrapTests {
    @Test("converts the persisted profile into one HTTP/3-only immutable snapshot")
    func createsStrictHTTP3Snapshot() throws {
    ...
    @Test("rejects malformed platform inputs before starting the native core")
    func rejectsMalformedInputs() throws {
        ...
        #expect(throws: StrictHTTP3PacketTunnelBootstrapError.invalidCredentialReference) {
            try StrictHTTP3PacketTunnelBootstrap(
        ...
        #expect(throws: StrictHTTP3PacketTunnelBootstrapError.invalidClientRoutes) {
            try StrictHTTP3PacketTunnelBootstrap(
