...
`performance`-профили автоматически используют адаптивный режим: два HTTPS
carrier-соединения прогреваются в фоне, третье создаётся только под нагрузкой и
закрывается после простоя. Каждое соединение независимо проходит NP/2
аутентификацию, согласование v2.2, шифрование и Mosaic; один TCP-поток между
соединениями не дробится.
