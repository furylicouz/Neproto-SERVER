...
During the bounded native-client HTTP/3 isolation phase, Windows and the native
iOS Packet Tunnel use `carrier_policy=http3-only` and one carrier session. This
policy permits only the configured HTTP/3/WebTransport carrier for initial
connection and same-carrier reconnection. HTTPS/WebSocket and WebRTC routes
remain stored for a later rollback but are never passed to the strict runtime
or dialed as fallbacks. If HTTP/3 is unavailable, the client reports the
HTTP/3 failure instead of silently changing transport. Generic desktop clients
retain the normal adaptive policy.

The native iOS provider owns an instance-scoped strict core rather than the
legacy process-global adaptive runtime. Its bounded diagnostic snapshot reports
QUIC RTT, packet/byte counters, and loss without destinations or payloads. A
Wi-Fi/cellular change first probes the current authenticated HTTP/3 session and
may replace it only with another authenticated HTTP/3 session.
...
3. Clients using the adaptive policy reuse it for the initial connection, warm
   carrier pool, and carrier migration. They must not pin a media session to
   HTTPS merely because HTTPS authenticated first on a prior network. Strict
   HTTP/3 clients never construct the adaptive pool.
