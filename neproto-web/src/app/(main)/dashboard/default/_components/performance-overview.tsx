...
export function PerformanceOverview({ locale }: { locale: AppLocale }) {
  const messages = APP_MESSAGES[locale].dashboard.performance;
  const dateLocale = localeTag(locale);
  const chartConfig = {
    newCustomers: {
      label: messages.newCustomers,
      color: "var(--chart-1)",
    },
    activeAccounts: {
      label: messages.activeAccounts,
      color: "var(--chart-2)",
    },
    returningUsers: {
      label: messages.returningUsers,
      color: "var(--chart-3)",
    },
  ...
  return (
    <Card className="@container/card">
      <CardHeader>
        ...
        <CardDescription>
          <span className="@[540px]/card:block hidden">{messages.description}</span>
          <span className="@[540px]/card:hidden">{messages.compactDescription}</span>
        </CardDescription>
        <CardAction className="flex items-center gap-2">
          <Select defaultValue="quarter">
            <SelectTrigger size="sm" className="w-28">
              <SelectValue placeholder={messages.threeMonths} />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
                <SelectLabel>{messages.period}</SelectLabel>
                <SelectItem value="quarter">{messages.threeMonths}</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
          ...
          <Select defaultValue="all">
            <SelectTrigger size="sm" className="w-32">
              <SelectValue placeholder={messages.allSegments} />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
                <SelectLabel>{messages.segments}</SelectLabel>
                <SelectItem value="all">{messages.allSegments}</SelectItem>
                <SelectItem value="paid">{messages.paid}</SelectItem>
                <SelectItem value="organic">{messages.organic}</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
        ...
        <ChartContainer config={chartConfig} className="aspect-auto h-80 w-full">
          <ComposedChart data={chartData} margin={{ top: 0 }}>
            ...
            <XAxis
              ...
              minTickGap={48}
              tickFormatter={(value) =>
                parseISO(value).toLocaleDateString(dateLocale, {
                  month: "short",
                  day: "numeric",
                })
              }
